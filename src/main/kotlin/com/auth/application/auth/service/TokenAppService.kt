...
class TokenAppService(
     ...
     */
    @Transactional
    fun generateTokens(userInfo: UserTokenInfo): TokenDto = with(userInfo) {
     ...
     */
    @Transactional
    fun generateTokens(user: User): TokenDto = with(user) {
        generateTokensInternal(
            subject = email.value,
            userId = id,
            roles = roles
    ...
    fun revokeRefreshToken(token: String): Boolean = 
        refreshTokenDomainService.revokeToken(token) != null
    ...
    fun revokeAllUserTokens(userId: Long): Int = 
        refreshTokenDomainService.revokeAllUserTokens(userId)
    ...
    fun generateOneTimeToken(email: String, userId: Long, purpose: TokenPurpose): String =
        tokenGenerator.generateOneTimeTokenString(email, userId, purpose)
